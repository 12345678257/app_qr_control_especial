...
# ---- logo centrado con marco blanco redondeado ----
...
FIELDS = [
    "nombre_generico","concentracion","forma_farmaceutica","presentacion","control",
...
with st.expander("🧩 Generar QR individual", expanded=True):
    ...
    if st.button("🔲 Generar QR individual", type="primary"):
        ...
        row.update({
            "nombre_generico":nombre, "concentracion":conc, "forma_farmaceutica":forma,
...
if uploaded is not None:
    # Leer el CSV a memoria y crear 'rows'
    try:
        text = uploaded.getvalue().decode("utf-8-sig")
    except Exception:
        text = uploaded.getvalue().decode("latin-1")
    reader = csv.DictReader(text.splitlines())
    missing = [c for c in tpl_cols if c not in (reader.fieldnames or [])]

    if missing:
        st.error("❌ Faltan columnas: " + ", ".join(missing))
    else:
        rows = list(reader)  # ← AHORA SÍ existe 'rows'
        st.success(f"✅ {len(rows)} filas")
        ...
        if st.button("🔲 Generar ZIP con QRs", type="primary"):
            ...
            with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
                for i, r in enumerate(rows):
                    ...
                    img = make_qr(payload, ecc=ecc_for_batch,
