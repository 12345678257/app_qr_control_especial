# -*- coding: utf-8 -*-
"""
Capturador QR → Excel (Tabulado) — v2.1
- CSV con delimitador ';' y QUOTE_ALL (evita desalineación por comas en texto)
- Manejo de error si el XLSX está abierto/bloqueado
"""
...
def ensure_workbook(path: Path):
    if path.exists():
        try:
            ...
            pass
    wb = Workbook()
    ws = wb.active; ws.title = "Escaneos"; ws.append(COLUMNS)
    return wb, ws
...
def normalize_key(s: str) -> str:
    ...
    s = re.sub(r"\s+", "_", s); s = re.sub(r"[^a-z0-9_]", "", s)
    return s
...
def parse_data_url(text: str) -> dict:
    if text.startswith("data:text/html;base64,"):
        ...
        return {}
    rows = re.findall(r"<tr>\s*<th[^>]*>(.*?)</th>\s*<td[^>]*>(.*?)</td>\s*</tr>", html_txt, flags=re.I|re.S)
    out = {}
    for th, td in rows:
        key = html.unescape(re.sub("<.*?>","", th)).strip()
        val = html.unescape(re.sub("<.*?>","", td)).strip()
...
class App:
    def __init__(self, root):
        self.root = root
        root.title("QR → Excel (Tabulado) — v2.1")
        ...
        self.path_var = tk.StringVar(value=str(DEFAULT_XLSX.resolve()))
        ...
        ttk.Checkbutton(top, text="También guardar CSV (; delimitado)", variable=self.csv_var).pack(side="left", padx=10)
        ...
        self.csv_path = DEFAULT_CSV if self.xlsx_path.name.endswith(".xlsx") else Path(str(self.xlsx_path)+".csv")
    ...
    def on_enter(self, event=None):
        ...
        self.ws.append([row[c] for c in COLUMNS])
        try:
            self.wb.save(self.xlsx_path)
        except PermissionError:
            messagebox.showerror("Archivo bloqueado",
                                 f"No se pudo guardar {self.xlsx_path}.\n¿Está abierto en Excel? Ciérrelo y reintente.")
            return
