# -*- coding: utf-8 -*-
'''
Generador de QR Medicamentos — v3.3 (Solo QR) — FIX
- Genera únicamente códigos QR con información completa del medicamento
- Modos QR: Mini página (tabla), Texto legible, JSON, URL
- Auto-fallback para evitar error "version 41"
'''
...
# capacidad aproximada (bytes, versión 40) por ECC en modo binario
CAP_BYTES = {
    "Q (25%)": 1850,
    "M (15%)": 2200,
    "L (7%)": 2800,
...
def make_qr(text: str, box_size: int, border: int, fill_color: str, ecc_label: str) -> Image.Image:
...
def build_text_payload(d: Dict[str, Any], compact: bool = False) -> str:
    '''Genera texto legible con información del medicamento'''
    if compact:
        L = [
...
<table>'''
    ...
    return "data:text/html," + _u.quote(html, safe="/:;,+-_.!~*'()#?=&")
...
def safe_make_qr(payload: str, fill_color: str, ecc_first: str = "Q (25%)", box_size: int = 10, border: int = 4) -> Tuple[Image.Image, str]:
...
def render_qr_only(data: Dict[str, Any], qr_mode: str, qr_color: str, ecc_choice: str,
                   qr_size: int = 512, border: int = 4) -> Tuple[Image.Image, str, str]:
        ...
        try:
            ...
            if st.button("🔲 Generar QRs por lotes", type="primary"):
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    ...
                    for idx, row in df.iterrows():
                        try:
                            ...
                            row_data.setdefault('control', 'Psicotrópico')
                            row_data.setdefault('url_qr', "")
                            ...
                            st.warning(f"⚠️ Error en fila {idx + 1}: {str(e)}")
                            error_count += 1
                ...
                if success_count > 0:
                    ...
                    if error_count > 0:
                        st.warning(f"⚠️ {error_count} errores durante el procesamiento")
